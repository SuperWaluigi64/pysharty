...
def download_json(local_filename, url, clobber=False):
    ...
    with open(local_filename, 'w') as json_file:
        json_file.write(json.dumps(requests.get(url).text, sort_keys=True, indent=2, separators=(',', ': ')))

def main():
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Quick and dirty soyjak.party Archiver")
        print("%s - Save the JSON and all images for an soyjak.party post." % (sys.argv[0]))
    ...
    board = pysharty.Board(board_name)
    thread = board.get_thread(thread_id)
    
    # create folders according to chan.arc standard
    path = os.path.join(os.getcwd(), "soyjak_party", board_name, thread_id)
    images_path = os.path.join(path, "images")
    mkdirs(images_path)

    # archive the thread JSON
    url_builder = pysharty.Url(board_name)
